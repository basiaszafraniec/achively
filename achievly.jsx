...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.storage.get('achievly-v2').then(r => {
  ...
  async function save(next) {
    setGoals(next);
    try { await window.storage.set('achievly-v2', JSON.stringify(next)); } catch {}
  }
  ...
  if (!loaded) return (
    <div style={{ ...ff, minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center',
...
function CreateGoalModal({ form, setForm, onSave, onClose, TF, ff }) {
  ...
  const overlayStyle = { position:'fixed', inset:0, background:'rgba(20,10,30,0.35)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:500, backdropFilter:'blur(6px)' };
  const sheetStyle  = { background:'#fff', borderRadius:'28px 28px 0 0', padding:'28px 24px 32px', width:'100%', maxWidth:520, maxHeight:'88vh', overflowY:'auto', ...ff };
